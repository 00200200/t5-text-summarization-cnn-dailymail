...
def generate_examples(model, dataset_name, num_examples=3):
    ...
    dataset = load_dataset(path, version, split="test") if version else load_dataset(path, split="test")
...
def train_dataset(dataset_name, train_size=30000, val_size=1000, batch_size=16, epochs=5):
    print(f"\n🚀 Training on {dataset_name.upper()}")
    print(f"📊 {train_size} train samples, batch_size={batch_size}, epochs={epochs}")
    
    model = SummarizerModel()
    train_loader, val_loader = get_data_loaders(dataset_name, batch_size, train_size, val_size, model.tokenizer)
